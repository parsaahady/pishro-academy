...
- Use HTTPS and set `cookie_secure` to `true` (this also enables the `Strict-Transport-Security` header automatically).
- Disable `setup_enabled` and delete/rename `api/setup.php` immediately after the first admin account is created.
- Set `setup_token` in `api/config.php` to a long, random value before deploying; never leave the placeholder value from `config.example.php` in place.
- Use a strong, unique admin password (12+ characters, letters and numbers, ideally generated by a password manager). There is no default/hardcoded admin login anywhere in this codebase — always create the account through `api/setup.php`.
- Keep the database credentials private and use a dedicated MySQL user with the minimum required privileges (SELECT/INSERT/UPDATE/DELETE on the app schema only).
- Keep upload directories protected from script execution — `uploads/players`, `uploads/blogs`, and `uploads/coaches` each ship with an `.htaccess` that disables PHP/CGI execution as defense-in-depth, in addition to the strict MIME/extension checks performed server-side on every upload.
- Login attempts, comment submissions, and contact-form submissions are all rate-limited server-side (in the database), so the limits hold even if a client clears cookies or opens a fresh session.
- Review and approve comments before publication.
- Back up the database and upload directories regularly.
- Run the site behind a Web Application Firewall / reverse proxy rate limiter if it is a high-value target, as an additional layer on top of the application-level protections above.
